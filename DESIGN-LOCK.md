...
| Commit | Fecha | Locked |
|---|---|---|
| `887d0bd` | 2026-05-16 | **Programa de delegaciones por país** (decisión Teo A1/B1/C1/D2): nueva tabla `delegations` (code legible BOCALAN-MX, welcome_bonus_tokens=3, commission_pct_web=10, commission_pct_ios=5), nueva FK nullable `users.delegation_id` inmutable. `/auth/register` resuelve invite_code orden 1)delegations 2)AMBASSADOR_CODE 3)default. Endpoints admin: GET/POST/PATCH /admin/delegations + GET /admin/delegations/report con agregados (users, paying_users, revenue, commission) sin exponer datos individuales. Frontend toast personalizado con delegation_name. Backwards-compat 100%. SW v135. **STAGING ONLY**, PROD pendiente E2E. Smoke test: 401 admin endpoints + 200 register con código inválido devolviendo tokens=5 delegation_name=null (fallback correcto). |
| `2e4f8a7` | 2026-05-16 | **Opción C lang por caso** (decisión Teo): `Case.lang VARCHAR(2) NULLABLE`, endpoints plan-simple/abc-explained/daily-followup-today usan `effective_lang = case.lang \|\| query_lang \|\| 'es'`, daily-followup/init hace backfill oportunista, frontend estampa lang al crear record + envía en migrate + pinta badge sutil EN/ES en cards de s-records cuando difiere del UI lang. Backwards-compat 100%: casos legacy `case.lang=NULL` mantienen comportamiento previo. SW v134. **STAGING ONLY (beta.thedogsmind.net), PROD a la espera de verificación E2E de Teo en su iPhone**. Smoke test badge UI 3-escenarios PASS. |
