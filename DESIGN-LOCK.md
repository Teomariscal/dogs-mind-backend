...
| `877d664` | 2026-05-17 | **Vídeo soft-truncate UX** (10s analyze window, 20s tope superior). `extract_frames` acepta `max_duration_sec` (OpenCV fps×duration, ffmpeg `-t`). Notice inline verde "Tu vídeo dura Xs — analizaremos los primeros 10s" cuando duración > 10s. SW v142. **PROD**. |
| `c3b042d` | 2026-05-17 | **Activar upload vídeo funcional** en anamnesis (sustituye teaser BETA visual por drop-zone + input file + preview + remove). Reusa JS attachVideo/handleVideoSelect/handleVideoDrop ya existentes. i18n anam_video_remove. SW v141. **PROD**. |
